...
void saveXML(CvIntHaarFeatures* haarFeatures,int stage,vector<MyStumpClassifier> strongClassifier,const char* dirname,float stage_thresold)
{
	...
	{
		float error = strongClassifier[j].error;
		float b = error / (1 - error);
		float temp = log(1 / b);
		sum = sum + temp;
	}
	sum = sum * stage_thresold;
	root->SetAttribute("stage_thresold", sum);
	char rectStr[200];
	...
	//	sunElement1->SetText(weakClassifier.compidx);
		...
		float error = weakClassifier.error;
		float b = error / (1 - error);
		float temp = log(1 / b);
...
void combineXml(int equalweights, int npos, int nneg,float minhitrate, float maxfalsealarm, const char* dirname,MySize winsize)
{
	...
	XMLElement* minhitrateNode = doc2.NewElement("minhitrate");
	minhitrateNode->SetText(minhitrate);
	root2->InsertEndChild(minhitrateNode);
	XMLElement* maxfalsealarmNode = doc2.NewElement("maxfalsealarm");
	maxfalsealarmNode->SetText(maxfalsealarm);
	root2->InsertEndChild(maxfalsealarmNode);
	XMLElement* equalweightsNode = doc2.NewElement("equalweights");
	equalweightsNode->SetText(equalweights);
	root2->InsertEndChild(equalweightsNode);
	XMLElement* nposNode = doc2.NewElement("npos");
	nposNode->SetText(npos);
	root2->InsertEndChild(nposNode);
	XMLElement* nnegNode = doc2.NewElement("nneg");
	nnegNode->SetText(nneg);
	root2->InsertEndChild(nnegNode);
